...
from Ch06.utils.plotSupportVectors import plotSVM
...
def calcEk(oS, k_):
    fXk = float(multiply(oS.alphas, oS.labelMat).T * oS.K[:, k_] + oS.b)
    Ek = fXk - float(oS.labelMat[k_])
    return Ek
...
def updateEk(oS, k_):  # after any alpha has changed update the new value in the cache
    Ek = calcEk(oS, k_)
    oS.eCache[k_] = [1, Ek]
...
def calcWs(alphas, dataArr, classLabels):
    ...
    w_ = zeros((n, 1))
    for i in range(m):
        w_ += multiply(alphas[i] * labelMat[i], X[i, :].T)
    return w_
...
def testDigits(kTup=("rbf", 10)):
    dataArr, labelArr = loadImages("./digits")
...
def calcEkK(oS, k_):
    fXk = float(multiply(oS.alphas, oS.labelMat).T * (oS.X * oS.X[k_, :].T)) + oS.b
    Ek = fXk - float(oS.labelMat[k_])
    return Ek
...
def innerLK(i, oS):
    ...
    if ((oS.labelMat[i] * Ei < -oS.tol) and (oS.alphas[i] < oS.C)) or (
        ...
        if L == H:
            ...
            return 0
        eta = (
            2.0 * oS.X[i, :] * oS.X[j, :].T - oS.X[i, :] * oS.X[i, :].T - oS.X[j, :] * oS.X[j, :].T
        )
        if eta >= 0:
            print("eta>=0")
            return 0
...
def main():
    ...
    # from os import listdir
    #
    # filename = listdir("digits/trainingDigits")
    # print(filename)
